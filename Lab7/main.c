...
/**
 * Function to calculate the average expense amount
 *
 * @return The average expense amount, or 0.0 if there are no expenses
 */
double calculateAverageExpense() {
    if (expenseCount == 0) {
        return 0.0;
    }
    return calculateTotalExpenses() / expenseCount;
}

/**
 * Function to get the highest expense amount
 *
 * @return The amount of the largest expense, or 0.0 if there are no expenses
 */
double calculateHighestExpense() {
    if (expenseCount == 0) {
        return 0.0;
    }
...
void updateUI() {
    ...
    // Call JavaScript function to update the average expense display
    double average = calculateAverageExpense();
    EM_ASM({
        updateAverageExpense($0);
    }, average);
    
    // Call JavaScript function to update the highest expense display
    double highest = calculateHighestExpense();
    EM_ASM({
        updateHighestExpense($0);
    }, highest);
